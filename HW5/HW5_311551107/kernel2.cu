...
__global__ void mandelKernel(int* cuda_data, float lowerX, float lowerY, float stepX, float stepY, int width, int maxIterations, int pitch) {
...
void hostFE (float upperX, float upperY, float lowerX, float lowerY, int* img, int resX, int resY, int maxIterations)
{
    ...
    mandelKernel<<<numBlock, threadsPerBlock>>>(cuda_data, lowerX, lowerY, stepX, stepY, resX, maxIterations, pitch);
